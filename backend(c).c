...
int validate() 
{ 
	// check whether the name written is valid or not
	...
	if (flag == 1) { 
		count = 0; 
		// check whether email id is valid r not
...
{ 
	...
	// in variables 
	strcpy(m[1].shop, "SHOES"); 
	strcpy(m[1].item1, "NIKE Air Force"); 
	strcpy(m[1].item2, "Reebok"); 
	strcpy(m[1].item3, "one8x puma"); 
	m[1].first = 13999; 
	m[1].second = 6999; 
	m[1].third = 5999; 

	strcpy(m[2].shop, "Watches"); 
	strcpy(m[2].item1, "TITAN"); 
	strcpy(m[2].item2, "RADO"); 
	strcpy(m[2].item3, "FASTRACK"); 
	m[2].first = 7999; 
	m[2].second = 2000; 
	m[2].third = 1499; 

	strcpy(m[3].shop, "MENS"); 
	strcpy(m[3].item1, "SHIRTS"); 
